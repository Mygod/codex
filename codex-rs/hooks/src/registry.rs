...
pub struct HooksConfig {
    ...
    pub legacy_notify_events: Vec<String>,
...
impl Default for HooksConfig {
    fn default() -> Self {
        Self {
            ...
            legacy_notify_events: crate::user_notification::default_notify_events(),
...
pub struct Hooks {
    ...
    after_input_requested: Vec<Hook>,
    after_elicitation_requested: Vec<Hook>,
...
impl Hooks {
    pub fn new(config: HooksConfig) -> Self {
        ...
        let legacy_notify_hook = legacy_notify_argv
            .filter(|argv| !argv.is_empty() && !argv[0].is_empty())
            .map(|argv| {
                crate::user_notification::notify_hook_with_events(argv, legacy_notify_events)
            });
        let after_agent = legacy_notify_hook.clone().into_iter().collect();
        ...
        Self {
            ...
            after_approval_requested: legacy_notify_hook.clone().into_iter().collect(),
            after_input_requested: legacy_notify_hook.clone().into_iter().collect(),
            after_elicitation_requested: legacy_notify_hook.into_iter().collect(),
            engine,
        }
    }
    ...
    fn hooks_for_event(&self, hook_event: &HookEvent) -> &[Hook] {
        match hook_event {
            ...
            HookEvent::AfterInputRequested { .. } => &self.after_input_requested,
            HookEvent::AfterElicitationRequested { .. } => &self.after_elicitation_requested,
        }
    }
